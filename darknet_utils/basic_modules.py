import glob
import parse_darknet_cfg
import operator
import numpy as np
import torch
import functools
from torch import nn
from typing import Dict, Tuple, Optional, NamedTuple, Type, List, Iterable, Sequence
from collections import defaultdict

int_or_none = Optional[int]
int_or_none_tuple = Tuple[int_or_none, ...]
...
TYPE2MODULE: Dict[str, Type[nn.Module]] = dict()
...
def compute_numel(shape: int_or_none_tuple) -> int:
...
class DarknetConvolution(nn.Sequential):
    @staticmethod
    def compute_output_shape(input_shape: int_or_none_tuple,
                             ksize: int, stride: int, pad: int, out_ch: int) -> int_or_none_tuple:
        _, in_h, in_w = input_shape
        out_h = None if in_h is None else (
                                                  in_h + 2 * pad - ksize) // stride + 1
        out_w = None if in_w is None else (
                                                  in_w + 2 * pad - ksize) // stride + 1
        return out_ch, out_h, out_w

    def __init__(self, input_shape: int_or_none_tuple, options: Dict[str, str]):
        ...
        self.output_shape: int_or_none_tuple = self.compute_output_shape(input_shape,
        ...
        self.conv = nn.Conv2d(in_ch, out_ch, (ksize, ksize),
                              (stride, stride), padding, bias=True)
        self.use_bn = use_bn
...
class DarknetMaxPool(nn.Sequential):
    @staticmethod
    def compute_output_shape(input_shape: int_or_none_tuple,
                             ksize: int, stride: int, padding: int) -> int_or_none_tuple:
    ...
    def __init__(self, input_shape: int_or_none_tuple, options: Dict[str, str]):
        ...
        self.output_shape: int_or_none_tuple = self.compute_output_shape(
...
class DarknetConnected(nn.Sequential):
    def __init__(self, input_shape: Tuple[int, ...], options: Dict[str, str]):
        ...
        # use_bn = int(options.get('batch_normalize', 0))
        self.output_shape = (output_numel,)
...
class DarknetRoute(nn.Module):
    ...
    def __init__(self, shapes: Sequence[int_or_none_tuple], current_index: int, options: Dict[str, str]):
        super().__init__()
    ...
    def forward(self, XList: Sequence[torch.Tensor]) -> torch.Tensor:
        if len(XList) == 1:
            return XList[0]
...
class DarknetReorg(nn.Module):
    def __init__(self, input_shape: int_or_none_tuple, options: Dict[str, str]):
        ...
        in_ch, in_h, in_w = input_shape
        self.output_shape = (in_ch * self.stride * self.stride, in_h // self.stride, in_w // self.stride)
...
class DarknetInferenceModel(nn.Module):
    def __init__(self, input_shape: int_or_none_tuple, section_iter: Iterable[parse_darknet_cfg.Section], fp):
        super().__init__()
        tensor_shapes = []
...
def BuildDarknetInferenceModel(cfg_file: str, weight_file: str) -> DarknetModel:
    all_sections = parse_darknet_cfg.parse_darknet_cfg(cfg_file)
    with open(weight_file, 'rb') as fp:
        ...
        model = DarknetInferenceModel(input_shape, section_iter, fp)
        model.eval()
