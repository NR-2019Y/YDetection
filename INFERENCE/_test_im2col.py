...
conv_cpp = ctypes.cdll.LoadLibrary("libconv.so")
...
def im2col_np(img: np.ndarray, kh: int, kw: int, sh: int, sw: int, dh: int, dw: int,
    ...
    return np.lib.stride_tricks.as_strided(img, (batch_size, oh, ow, ic, kh, kw),
...
def im2col_cpp(img: np.ndarray, kh: int, kw: int, sh: int, sw: int, dh: int, dw: int,
    ...
    result = np.zeros((batch_size, oh, ow, ic, kh, kw), dtype=np.float32)

    c_float_p = ctypes.POINTER(ctypes.c_float)
    conv_cpp.im2col(img.ctypes.data_as(c_float_p), result.ctypes.data_as(c_float_p),
